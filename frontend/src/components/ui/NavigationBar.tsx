...
const Icon = ({ name, active }: { name: 'home' | 'profile' | 'settings'; active: boolean }) => {
  const common = `w-5 h-5 ${active ? 'text-blue-600' : 'text-slate-500'}`
...
export default function NavigationBar({ activePath }: Props) {
  ...
  return (
    <footer className="bg-white/80 border-t border-slate-200 backdrop-blur">
      <div className="max-w-md mx-auto px-4 py-3">
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm px-2 py-2 flex items-center justify-around">
        {items.map(item => {
          const active = activePath === item.to
          return (
            <Link key={item.key} to={item.to} className={`flex flex-col items-center justify-center gap-1 py-2 px-3 rounded-xl ${active ? 'bg-slate-50' : 'hover:bg-slate-50'}`}>
              <Icon name={item.key} active={active} />
              <span className={`text-xs ${active ? 'text-blue-600 font-medium' : 'text-slate-500'}`}>{item.label}</span>
            </Link>
          )
        })}
        </div>
